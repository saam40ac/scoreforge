...
export default async function AdminAnalyticsPage() {
  // Verifica autenticazione con client normale
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return null

  // Usa service role per bypassare RLS su analytics_events
  ...
  const { data: topPDataRaw } = topPIds.length > 0
    ? await admin.from('portfolios').select('id, title, slug, owner_id').in('id', topPIds.map(([id]) => id))
    : { data: [] }
  const topPData = (topPDataRaw || []) as { id: string; title: string; slug: string; owner_id: string }[]

  const { data: topOwnersRaw } = topPData.length > 0
    ? await admin.from('profiles').select('id, name').in('id', topPData.map(p => p.owner_id))
    : { data: [] }
  ...
  const totalViews = dailyEvents.filter(e => e.event_type === 'view').length
  const totalPlays = dailyEvents.filter(e => e.event_type === 'play').length
  const totalGeo   = geoData.length
    ...
    <div style={{ padding: '32px', fontFamily: "'Outfit', sans-serif", color: '#f0ebe0' }}>
        ...
        <div style={{ display: 'flex', gap: '3px', alignItems: 'flex-end', height: '80px', marginBottom: '8px' }}>
          {chartData.map((d, i) => (
            <div key={i} title={`${d.date}: ${d.views}v ${d.plays}p`} style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '1px', alignItems: 'center', height: '100%', justifyContent: 'flex-end' }}>
              <div style={{ width: '45%', background: '#7f77dd', opacity: .8, borderRadius: '1px 1px 0 0', height: `${Math.round((d.views / maxVal) * 76)}px`, minHeight: d.views > 0 ? '2px' : '0' }} />
              <div style={{ width: '45%', background: '#c8a45a', opacity: .8, borderRadius: '1px 1px 0 0', height: `${Math.round((d.plays / maxVal) * 76)}px`, minHeight: d.plays > 0 ? '2px' : '0', marginLeft: '1px' }} />
            </div>
          ))}
        </div>
        <div style={{ display: 'flex', gap: '16px', fontSize: '10px', color: '#5a5548' }}>
          <span style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
            <span style={{ width: '8px', height: '8px', background: '#7f77dd', borderRadius: '1px', display: 'inline-block' }} />Views
          </span>
          <span style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
            <span style={{ width: '8px', height: '8px', background: '#c8a45a', borderRadius: '1px', display: 'inline-block' }} />Ascolti
          </span>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '14px' }}>
        ...
        <div style={{ background: '#0f0f1a', border: '1px solid #1e1e2e', borderRadius: '12px', padding: '20px' }}>
          ...
          {topPIds.length === 0 && <div style={{ fontSize: '13px', color: '#5a5548' }}>Nessun dato ancora.</div>}
        ...
        <div style={{ background: '#0f0f1a', border: '1px solid #1e1e2e', borderRadius: '12px', padding: '20px' }}>
          ...
          {topGeo.length === 0 && <div style={{ fontSize: '13px', color: '#5a5548' }}>Nessun dato ancora.</div>}
        ...
        <div style={{ background: '#0f0f1a', border: '1px solid #1e1e2e', borderRadius: '12px', padding: '20px' }}>
          ...
          {topBrowsers.length === 0 && <div style={{ fontSize: '13px', color: '#5a5548' }}>Nessun dato ancora.</div>}
